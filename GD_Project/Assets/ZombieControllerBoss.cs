...
public class ZombieControllerBoss : MonoBehaviour
{
    ...
    {   
        ...
        if(distance <= detectRadius){
            ...
            Debug.Log("Check distance" + distance);
              ...
              //check if the player is inside than 5.5 distance to activate attack animation 
             if (distance < 5.5){
                // animator.SetBool("isRange",true);q
                animator.SetBool("inRange", true);
                Debug.Log("Check close" + distance);
                 
            }
            else{
                // if the player is under detect radius but not inside 5.5 radius it will start walk towards player animation
    ...
    //this gives visual representation of Boss enemy detect radius
